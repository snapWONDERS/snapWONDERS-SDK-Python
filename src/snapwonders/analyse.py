...
class AnalyseSession:
    ...
    def __init__(self, transport: HttpTransport, upload_uid: str, max_upload_bytes: int | None = None) -> None:
        self._t = transport
        self.upload_uid = upload_uid
        # Reported by the server at session creation. Kept so the direct-upload cap can move
    ...
    def upload(self, file_path: str | os.PathLike[str]) -> str:
        return upload_router.upload_file(
            self._t, file_path,
            upload_uid=self.upload_uid, step=1,
...
class Analyse:
    ...
    def create_session(self) -> AnalyseSession:
        ...
        return AnalyseSession(self._t, data["upload_uid"], data.get("max_upload_bytes"))
