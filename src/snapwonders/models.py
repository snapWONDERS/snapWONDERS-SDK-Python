...
class ResultFile:
    ...
    def download(self, dest: str | os.PathLike[str]) -> Path:
        ...
        if raw.endswith(("/", "\\")) or target.is_dir():
            # Take only the final component of the server-supplied name. `name` arrives in API
            # JSON, and a value containing separators or `..` would otherwise let the response
            # decide where on the caller's disk this lands. The server is first-party, so this
            # is hardening rather than a known hole — but a client library that writes files
            # should not let a remote value choose the directory.
            target = target / Path(self.name).name
...
class AnalyseItem:
    ...
    #: findings), ``steganography`` (hidden-content check), ``forensic`` (ELA / noise-inconsistency
    #: tile counts), and — video only — ``audio_splice`` and ``video_tamper`` (inter-frame tamper,
    #: frame duplication, P-frame FFT, GPS timezone triangle). A plain dict — the exact keys grow
    #: over time; read what you need.
